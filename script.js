...
addTuitionClass() {
    ...
    if (classes.length >= 5) {
        return this.showToast('Maximum 5 tuition classes allowed');
    }
    ...
    if (!name || isNaN(day) || !time || !duration) {
        return this.showToast('Class added successfully!');
    }
    const newClass = {
    ...
    if (!this.data.tuitionClasses) this.data.tuitionClasses = [];
...
renderTuitionDashboard() {
    ...
    const days = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];
    if (classes.length === 0) {
        list.innerHTML = '<p class="text-muted text-center py-3">No classes scheduled.</p>';
        return;
    }
    list.innerHTML = classes.map(c => {
        const { isOngoing, countdown } = this.calculateClassStatus(c);
    ...
    if ((this.data.tuitionClasses || []).length > 3) {
        list.innerHTML += `<div class="view-all-tuition"><a href="tuition.html"><i class="fa fa-arrow-right"></i> View all classes</a></div>`;
    }
},
...
updateTuitionLimit() {
    ...
    if (limit) {
        ...
        limit.textContent = `${count} / 5 classes`;
    }
},
...
if (path.includes('tuition.html')) this.initTuition();
},
...
).join('') : '<p class="text-muted text-center py-3">No tasks today</p>';
...
cdList.innerHTML = '<p class="text-muted">No countdowns set. <a href="countdown.html">Add one</a></p>';
} else {
...
).join('') : '<p class="text-muted text-center py-3">No tasks for this date</p>';
},
...
this.data.tasks = this.data.tasks.filter(x => x.id !== id); this.save(); this.renderTasks(); this.showToast('Task deleted');
}
},
...
t.text = newText.trim(); this.save(); this.renderTasks(); this.showToast('Task updated!');
}
},
