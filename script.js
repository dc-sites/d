...
// 📅 Track which date is currently being viewed
...
initDashboard() {
// 📅 Updated to use YYYY-MM-DD format to match the new Tasks logic
...
// ✅ UPDATED: Per-day limit and Date Picker binding
initTasks() {
    const dateInput = document.getElementById('task-date-view');
    // Bind date selector
    if (dateInput) {
        dateInput.value = this.selectedTaskDate;
        dateInput.addEventListener('change', (e) => {
            this.selectedTaskDate = e.target.value;
            this.renderTasks();
        });
    }
    ...
    document.getElementById('task-form')?.addEventListener('submit', e => {
        ...
        // Check limit PER DAY
        const tasksForDay = this.data.tasks.filter(t => t.date === this.selectedTaskDate);
        if (tasksForDay.length >= 10) return this.showToast('Max 10 tasks allowed per day');
        
        const t = { 
            id: Date.now().toString(), 
            text: document.getElementById('task-input').value.trim(), 
            time: document.getElementById('task-time-input').value, 
            date: this.selectedTaskDate, // Uses the selected date
            done: false 
        };
...
// ✅ UPDATED: Global % and Edit Button
renderTasks() {
    const tasks = this.data.tasks.filter(t => t.date === this.selectedTaskDate);
    const doneToday = tasks.filter(t => t.done).length;
    
    // Stats for the selected day
    document.getElementById('task-count').textContent = tasks.length;
    document.getElementById('pending-count').textContent = tasks.length - doneToday;
    document.getElementById('done-count').textContent = doneToday;
    
    // 📈 Global completion % (across ALL days)
    const totalAll = this.data.tasks.length;
    const doneAll = this.data.tasks.filter(t => t.done).length;
    const completionPercent = totalAll ? Math.round((doneAll / totalAll) * 100) : 0;
    document.getElementById('completion-rate').textContent = `${completionPercent}%`;
    
    // Update progress bar if it exists
    const progressBar = document.getElementById('progress-bar');
    if (progressBar) progressBar.style.width = `${completionPercent}%`;

    // Render list with Edit button
    document.getElementById('tasks-list').innerHTML = tasks.map(t =>
        `<div class="task-item ${t.done ? 'done' : ''}">
            ...
            <button class="task-edit" onclick="APP.editTask('${t.id}')" style="background:none;border:none;color:var(--warning, #f59e0b);cursor:pointer;margin-right:5px;"><i class="fa fa-edit"></i></button>
            <button class="task-delete" onclick="APP.deleteTask('${t.id}')" style="background:none;border:none;color:var(--danger, #ef4444);cursor:pointer;"><i class="fa fa-trash"></i></button>
        </div>`
    ).join('') || '<p style="text-align:center;color:var(--text-muted);padding:10px;">No tasks for this date.</p>';
},
// ✅ NEW: Edit Task Method
editTask(id) {
    const t = this.data.tasks.find(x => x.id === id);
    if (!t) return;
    const newText = prompt("Edit task description:", t.text);
    const newTime = prompt("Edit time (HH:MM):", t.time);
    if (newText !== null && newTime !== null) {
        t.text = newText.trim() || t.text;
        t.time = newTime.trim() || t.time;
        this.save();
        this.renderTasks();
